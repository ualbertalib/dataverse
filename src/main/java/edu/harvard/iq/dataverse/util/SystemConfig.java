...
public class SystemConfig {
    ...
    /**
     * A JVM option for the advertised fully qualified domain name (hostname) of the Dataverse installation, such as
     * "dataverse.example.com", which may differ from the hostname that the server knows itself as.
     *
    ...
    /**
     * A JVM option for specifying the "official" URL of the site. Unlike the FQDN option above, this would be a
     * complete URL, with the protocol, port number etc.
     */
    ...
    /**
     * A JVM option to override the number of minutes for which a password reset token is valid
     * ({@link #minutesUntilPasswordResetTokenExpires}).
     */
    private static final String PASSWORD_RESET_TIMEOUT_IN_MINUTES = "dataverse.auth.password-reset-timeout-in-minutes";

    /**
     * A common place to find the String for a sane Solr hostname:port combination.
     */
    private final String saneDefaultForSolrHostColonPort = "localhost:8983";

    /**
     * The default number of datafiles that we allow to be created through zip file upload.
     */
    ...
    public static final String getVersionFromManifest() {
        try {
            if (appVersionString == null) {
                IOUtils.readLines(SystemConfig.class.getResourceAsStream("/../../META-INF/MENIFEST.MF")).stream()
                    .forEach(s -> {
                        if (s.startsWith("Implementation-Version:")) {
                            appVersionString = s.split("\\s")[1];
                        } else if (s.startsWith("Implementation-Build:")) {
                            buildNumberString = " build: " + s.split("\\s")[1];
                        }
                    });
            }
        ...
        return appVersionString + buildNumberString;
    }

    public String getVersion(final boolean withBuildNumber) {

        if (appVersionString == null) {

            // The Version Number is no longer supplied in a .properties file - so
            // we can't just do
            // return BundleUtil.getStringFromBundle("version.number", null, ResourceBundle.getBundle("VersionNumber",
            // Locale.US));
            //
            ...
            if (fileUrl != null) {
                ...
                if (filePath != null) {
                    ...
                    mavenPropertiesFilePath =
                        filePath.concat("../../../META-INF/maven/edu.harvard.iq/dataverse/pom.properties");
                    ...
                    logger.warning(
                        "Null file path representation of the location of persistence.xml in the webapp root directory!");
                }
            } else {
    ...
    public String getSolrHostColonPort() {
        final String solrHostColonPort =
            settingsService.getValueForKey(SettingsServiceBean.Key.SolrHostColonPort, saneDefaultForSolrHostColonPort);
    ...
    public int getMinutesUntilConfirmEmailTokenExpires() {
        ...
        final SettingsServiceBean.Key key = SettingsServiceBean.Key.MinutesUntilConfirmEmailTokenExpires;
        final String valueFromDatabase = settingsService.getValueForKey(key);
        if (valueFromDatabase != null) {
            try {
                final int intFromDatabase = Integer.parseInt(valueFromDatabase);
                if (intFromDatabase > 0) {
                    return intFromDatabase;
                } else {
                    logger.info("Returning " + reasonableDefault + " for " + key
                        + " because value must be greater than zero, not \"" + intFromDatabase + "\".");
                }
    ...
    /**
     * The number of minutes for which a password reset token is valid. Can be overridden by
     * {@link #PASSWORD_RESET_TIMEOUT_IN_MINUTES}.
     */
    public static int getMinutesUntilPasswordResetTokenExpires() {
        final int reasonableDefault = 60;
        final String configuredValueAsString = System.getProperty(PASSWORD_RESET_TIMEOUT_IN_MINUTES);
        if (configuredValueAsString != null) {
            ...
            try {
                ...
                if (configuredValueAsInteger > 0) {
                    ...
                    logger.info(PASSWORD_RESET_TIMEOUT_IN_MINUTES + " is configured as a negative number \""
                        + configuredValueAsInteger + "\". Using default value instead: " + reasonableDefault);
                    return reasonableDefault;
                }
            } catch (final NumberFormatException ex) {
                logger.info(
                    "Unable to convert " + PASSWORD_RESET_TIMEOUT_IN_MINUTES + " from \"" + configuredValueAsString
     ...
     * hostname. If none of these options is set, defaults to the InetAddress.getLocalHOst() and https; These are legacy
     * JVM options. Will be eventualy replaced by the Settings Service configuration.
     */
    ...
    public String getGuidesBaseUrl() {
        final String saneDefault = "http://guides.dataverse.org";
        final String guidesBaseUrl = settingsService.getValueForKey(SettingsServiceBean.Key.GuidesBaseUrl, saneDefault);
        return guidesBaseUrl + "/" + getGuidesLanguage();
    }

    private String getGuidesLanguage() {
        final String saneDefault = "en";
        return saneDefault;
    }

    /**
     * Download-as-zip size limit. returns 0 if not specified; (the file zipper will then use the default value) set to
     * -1 to disable zip downloads.
     */

    public long getZipDownloadLimit() {
        final String zipLimitOption = settingsService.getValueForKey(SettingsServiceBean.Key.ZipDownloadLimit);
    ...
    public int getZipUploadFilesLimit() {
        final String limitOption = settingsService.getValueForKey(SettingsServiceBean.Key.ZipUploadFilesLimit);
        Integer limit = null;
    ...
    public long getThumbnailSizeLimitImage() {
        final long limit = getThumbnailSizeLimit("Image");
        return limit == 0 ? 5000000 : limit;
    }

    public long getThumbnailSizeLimitPDF() {
        final long limit = getThumbnailSizeLimit("PDF");
        return limit == 0 ? 500000 : limit;
    }

    public long getThumbnailSizeLimit(final String type) {
        String option = null;
    ...
    public boolean isThumbnailGenerationDisabledForType(final String type) {
    ...
    public String getApplicationTermsOfUse() {
        final String saneDefaultForAppTermsOfUse = "There are no Terms of Use for this Dataverse installation.";
        final String appTermsOfUse =
    ...
    public String getApiTermsOfUse() {
        final String saneDefaultForApiTermsOfUse = "There are no API Terms of Use for this Dataverse installation.";
        final String apiTermsOfUse =
    ...
    public String getApplicationPrivacyPolicyUrl() {
        final String saneDefaultForPrivacyPolicyUrl = null;
        final String appPrivacyPolicyUrl = settingsService
    ...
    public boolean myDataDoesNotUsePermissionDocs() {
        ...
        return settingsService.isTrueForKey(SettingsServiceBean.Key.MyDataDoesNotUseSolrPermissionDocs,
            safeDefaultIfKeyNotFound);
    }

    public boolean isFilesOnDatasetPageFromSolr() {
        final boolean safeDefaultIfKeyNotFound = false;
        return settingsService.isTrueForKey(SettingsServiceBean.Key.FilesOnDatasetPageFromSolr,
            safeDefaultIfKeyNotFound);
    }
    ...
    public Integer getSearchHighlightFragmentSize() {
        final String fragSize = settingsService.getValueForKey(SettingsServiceBean.Key.SearchHighlightFragmentSize);
        if (fragSize != null) {
            try {
                return new Integer(fragSize);
            } catch (final NumberFormatException nfe) {
                logger.info(
                    "Could not convert " + SettingsServiceBean.Key.SearchHighlightFragmentSize + " to int: " + nfe);
            }
        }
        return null;
    }
    ...
    public long getTabularIngestSizeLimit() {
        ...
        final String limitEntry = settingsService.getValueForKey(SettingsServiceBean.Key.TabularIngestSizeLimit);

        if (limitEntry != null) {
            try {
                final Long sizeOption = new Long(limitEntry);
                return sizeOption;
            } catch (final NumberFormatException nfe) {
    ...
    public boolean isOAIServerEnabled() {
        final boolean defaultResponse = false;
    ...
    public boolean isTimerServer() {
        final String optionValue = System.getProperty(JVM_TIMER_SERVER_OPTION);
